         ...
         $workboard = DB::table('workboard')->get();
             ...
             foreach($join as $labor){
                    ...
                    if($labor->specialty == 'Médico General'){
                        ...
                        $mg = '["'.$labor->latitude.','.$labor->longitude.', "'.$labor->name.'", "'.$labor->workplace.'","'.$labor->general_amount.'",'.json_encode($workArray).', "'.$labor->id.'", "'.$labor->dr.'",'.json_encode($cite).', "https://s3.amazonaws.com/abiliasf/iconoo_doc_verde-01.png",'.json_encode(array_unique($blocker)).']';
                        } else{
                        $mg = '["'.$labor->latitude.','.$labor->longitude.', "'.$labor->name.'", "'.$labor->workplace.'","'.$labor->general_amount.'",'.json_encode($workArray).', "'.$labor->id.'", "'.$labor->dr.'",'.json_encode($cite).', "'.$labor->profile_photo.'",'.json_encode(array_unique($blocker)).']';
                    }
                    }
                    else{
                    if(!$labor->profile_photo){
                          $it[] = '["'.$labor->specialty.'",'.$labor->latitude.','.$labor->longitude.', "'.$labor->name.'", "'.$labor->workplace.'","'.$labor->general_amount.'",'.json_encode($workArray).', "'.$labor->id.'", "'.$labor->dr.'",'.json_encode($cite).', "https://s3.amazonaws.com/abiliasf/iconoo_doc_verde-01.png",'.json_encode(array_unique($blocker)).']';

                        } else{
                    $it[] = '["'.$labor->specialty.'",'.$labor->latitude.','.$labor->longitude.', "'.$labor->name.'", "'.$labor->workplace.'","'.$labor->general_amount.'",'.json_encode($workArray).', "'.$labor->id.'", "'.$labor->dr.'",'.json_encode($cite).', "'.$labor->profile_photo.'",'.json_encode(array_unique($blocker)).']';
                            }
                    $sp[] = '["'.$labor->specialty.'"]';
                    $mg = '0';
   ...
   $profInfo = DB::table('professional_information')
    ...
    public function appointments(){
         ...
         $appointments = DB::table('medical_appointments')
           ...
           ->join('labor_information', 'medical_appointments.workplace', '=', 'labor_information.id')
           ->where('medical_appointments.user', '=', Auth::id())
           ->where('medical_appointments.when', '>', Carbon::now()->format('d-m-Y'))
